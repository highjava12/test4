 ...
 */
final public class RC2Crypt extends SymmetricCipher {
     ...
     */
    public void initEffectiveKeyBits(int effectiveKeyBits) {
        this.effectiveKeyBits = effectiveKeyBits;
    }

    public static void checkKey(String algorithm, int keyLength)
     ...
     */
    public void encryptBlock(byte[] in, int inOfs, byte[] out, int outOfs) {
    ...
    public void decryptBlock(byte[] in, int inOfs, byte[] out, int outOfs) {
        int R0 =  (in[inOfs    ] & 0xff)
               + ((in[inOfs + 1] & 0xff) << 8);
        int R1 =  (in[inOfs + 2] & 0xff)
               + ((in[inOfs + 3] & 0xff) << 8);
