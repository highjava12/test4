...
import com.neo.crypto.provider.engine.ARIAConstants;
import com.neo.crypto.provider.engine.ARIACrypt;
import com.neo.crypto.provider.engine.DESConstants;
import com.neo.crypto.provider.engine.DESCrypt;
import com.neo.crypto.provider.engine.DESedeCrypt;
import com.neo.crypto.provider.engine.SEED;
import com.neo.crypto.provider.engine.SEEDConstants;
import com.neo.crypto.provider.engine.SEEDCrypt;
...
final class PBECipherCore {
    ...
    private boolean useFixedIV = false;
    ...
    PBECipherCore(String cipherAlg) throws NoSuchAlgorithmException,
        ...
        blkSize = DESConstants.DES_BLOCK_SIZE;
        cipher.setMode("CBC");
     ...
     *
     */
    PBECipherCore(String cipherAlg,String hashAlg) throws NoSuchAlgorithmException,
        NoSuchPaddingException {
        algo = cipherAlg;
        if (algo.equals("DES")) {
            cipher = new CipherCore(new DESCrypt(),
            ...
            cipher = new CipherCore(new DESedeCrypt(),
                                    DESConstants.DES_BLOCK_SIZE);
            blkSize = DESConstants.DES_BLOCK_SIZE;
        } else if (algo.equals("SEED")) {
            cipher = new CipherCore(new SEEDCrypt(),SEEDConstants.SEED_BLOCK_SIZE);
            blkSize = SEEDConstants.SEED_BLOCK_SIZE;
        } else if (algo.equals("ARIA")) {
            cipher = new CipherCore(new ARIACrypt(),
            		ARIAConstants.ARIA_BLOCK_SIZE);
            blkSize = ARIAConstants.ARIA_BLOCK_SIZE;
        } else {
            throw new NoSuchAlgorithmException("No Cipher implementation " +
                                               "for PBEWith" +hashAlg +"And" + algo);
        }
        cipher.setMode("CBC");
        cipher.setPadding("PKCS5Padding");
       
        // get instance of MD5 or SHA1
        md = MessageDigest.getInstance(hashAlg);
    }
    
    PBECipherCore(String cipherAlg,String hashAlg,boolean useFixedIV) throws NoSuchAlgorithmException,
    NoSuchPaddingException {
    	this(cipherAlg,hashAlg);
     ...
     * decryption, where the IV is derived from a user-supfplied password.
     *
    ...
    AlgorithmParameters getParameters() {
        ...
        if (salt == null) {
            salt = new byte[blkSize];
            NeoJceEntries.RANDOM.nextBytes(salt);
        }
        PBEParameterSpec pbeSpec = new PBEParameterSpec(salt, iCount);
        try {
            params = AlgorithmParameters.getInstance("PBEWithMD5And" +
                (algo.equalsIgnoreCase("DES")? "DES":"TripleDES"), "NEO");
        } catch (NoSuchAlgorithmException nsae) {
            // should never happen
            throw new RuntimeException("NeoJCE called, but not configured");
...
//            if (salt.length != blkSize) {
//                throw new InvalidAlgorithmParameterException
//                    ("Salt must be " + blkSize +" bytes long");
//            }
    ...
    private byte[] deriveCipherKey(Key key) {
        ...
        if (algo.equals("DES")) {
    		...
    		result = new byte[32];
			System.arraycopy (toBeHashed, 0, result, 0, 16);
    		
			if (this.useFixedIV)
			{
				System.arraycopy ("0123456789012345".getBytes(), 0, result, 16, 16);
			}
			else 
			{
				byte[] ivSeed = new byte[4];
				System.arraycopy(toBeHashed, 16, ivSeed, 0, 4);
				java.util.Arrays.fill(toBeHashed, (byte)0x00);
				
    			md.update (ivSeed);
    			toBeHashed = md.digest ();
    			
    			System.arraycopy (toBeHashed, 0, result, 16, 16);
			}
			java.util.Arrays.fill(toBeHashed, (byte)0x00);
        }
