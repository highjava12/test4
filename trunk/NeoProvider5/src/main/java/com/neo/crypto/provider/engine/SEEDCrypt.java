 ...
 */
public class SEEDCrypt extends SymmetricCipher implements SEEDConstants
{
	...
	public void init(boolean decrypting, String algorithm, byte[] key)
        ...
        this.forEncrypting = decrypting ? false : true;
        wKey = createWorkingKey(key);		
	}

	@Override
	public void encryptBlock(byte[] plain, int plainOffset, byte[] cipher,
			int cipherOffset) {
		processBlock(plain,plainOffset,cipher,cipherOffset);
	}
	...
	public void decryptBlock(byte[] cipher, int cipherOffset, byte[] plain,
		...
		processBlock(cipher,cipherOffset,plain,plainOffset);
	}
    ...
    private int processBlock(byte[] in, int inOff, byte[] out, int outOff) throws IllegalArgumentException, IllegalStateException
    {
    	
//    	System.out.println("processBlock(" + inOff + "): " + StringUtils.bin2printstr(in));
        if (wKey == null)
        {
            throw new IllegalStateException("SEED engine not initialised");
        }

        if (inOff + SEED_BLOCK_SIZE > in.length)
        {
            throw new IllegalArgumentException("input buffer too short");
        }

        if (outOff + SEED_BLOCK_SIZE > out.length)
        {
...
//        System.out.println("processBlock(" + outOff + "): " + StringUtils.bin2printstr(out));
        return SEED_BLOCK_SIZE;
    }
