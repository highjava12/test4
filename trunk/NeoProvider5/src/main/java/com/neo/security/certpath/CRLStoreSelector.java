...
//import security.util.Debug;
//import security.util.DerInputStream;
//import security.x509.CRLNumberExtension;
//import security.x509.X500Name;
...
public class CRLStoreSelector implements CRLSelector {
	...
	public void addIssuerName(String name) throws IOException {
		...
		addIssuerNameInternal(name, getX500Principal(name, "RFC2253"));
	}
	...
	public void addIssuerName(byte [] name) throws IOException {
		...
		addIssuerNameInternal(name.clone(), getX500Principal(name));
	}
	...
	private static HashSet parseIssuerNames(Collection names) 
		...
		for (Iterator t = names.iterator(); t.hasNext(); ) {
			...
			if (nameObject instanceof String) {
				...
				x500Principals.add(getX500Principal((String)nameObject, "RFC2253"));
			} else {
				try {
					x500Principals.add(new X509Principal((byte[])nameObject));
...
//							+ "don't match[" + xcrl.getIssuerDN().getName() + "]");
//				}
				return false;
			}
		}
...
//				}
			...
			BigInteger crlNum;
			try {
				byte[] encoded = ASN1OctetString.getInstance(ASN1Object.fromByteArray(crlNumExtVal)).getOctets();
				crlNum = CRLNumber.getInstance(ASN1Object.fromByteArray(encoded)).getValue();
